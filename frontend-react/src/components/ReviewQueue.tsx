...
import { Sparkles, ChevronDown, ChevronUp, AlertCircle, CheckCircle, X } from 'lucide-react';
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-lg shadow-xl border border-slate-200 w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
                            <button
                                onClick={() => setActionModal(null)}
                                className="px-5 py-2.5 rounded text-[13px] font-bold text-slate-600 hover:bg-slate-200 transition-colors font-mono uppercase tracking-widest"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={() => setActionModal(null)}
                                className={`px-5 py-2.5 text-white rounded text-[13px] font-bold shadow-sm transition-colors font-mono uppercase tracking-widest ${actionModal.type === 'deactivate' ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'
                                    }`}
                            >
                                Confirm
                            </button>
